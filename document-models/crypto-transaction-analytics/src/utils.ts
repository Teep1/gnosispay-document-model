...
export function recalculateAnalytics(
  ...
  // Calculate averages
  const expenseTransactions = transactions.filter(t => t.valueOut && t.valueOut.amount > 0);
  ...
  const projectedMonthSpend = daysElapsed > 0 ? (currentMonthExpenses / daysElapsed) * daysInMonth : currentMonthExpenses;
  ...
  if (monthlyBudget && monthlyBudget > 0) {
    ...
    if (percentUsed >= threshold) {
      alerts.push(`Spending alert: You've used ${percentUsed.toFixed(1)}% of your monthly budget`);
    }
    if (projectedMonthSpend > monthlyBudget) {
      alerts.push(`Projection alert: At current rate, you'll exceed your monthly budget by ${(projectedMonthSpend - monthlyBudget).toFixed(2)} ${baseCurrency}`);
    }
  }
  ...
  if (previousMonthExpenses > 0) {
    ...
    if (percentChange > 20) {
      alerts.push(`Your spending is ${percentChange.toFixed(1)}% higher than last month`);
    }
  }
