  ...
  const tokenBalances = useMemo((): TokenBalanceCalc[] => {
    ...
    transactions.forEach((tx) => {
      if (tx.valueIn?.token && tx.valueIn.amount) {
        balances[tx.valueIn.token] = (balances[tx.valueIn.token] || 0) + tx.valueIn.amount;
      }
      if (tx.valueOut?.token && tx.valueOut.amount) {
        balances[tx.valueOut.token] = (balances[tx.valueOut.token] || 0) - tx.valueOut.amount;
      }
    });
    ...
    return Object.entries(balances)
      ...
      .map(([token, amount]) => ({
        ...
        currencyCode: token === "GBPe" ? "GBP" : token === "EURe" ? "EUR" : "USD",
      }))
  ...
  const renderAnalyticsView = () => (
    <div className="p-4 pb-24 space-y-6">
      ...
      <div className="text-center py-12">
        ...
        <p className="text-gray-400 text-sm mt-1">Charts, categories, and insights</p>
      </div>
    </div>
  );
  ...
  const renderBudgetView = () => (
    <div className="p-4 pb-24 space-y-6">
      ...
      <div className="text-center py-12">
        ...
        <p className="text-gray-400 text-sm mt-1">Set spending limits and track goals</p>
      </div>
    </div>
  );
