      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl p-5 text-white shadow-lg">
          ...
          <p className="text-2xl font-bold">
            {currencySymbol}
            <FormattedNumber
              value={monthlyStats.thisMonthTotal}
              decimalScale={2}
            />
          </p>
          <div className="flex items-center gap-1 mt-2">
            <span
              className={`text-xs font-medium ${monthlyStats.change > 0 ? "text-red-300" : "text-green-300"}`}
            >
              {monthlyStats.change > 0 ? "↑" : "↓"}{" "}
              {Math.abs(monthlyStats.change).toFixed(1)}%
            </span>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-lg border border-gray-100">
          ...
          <p className="text-2xl font-bold text-gray-900">
            {currencySymbol}
            <FormattedNumber
              value={categoryTotals.reduce((sum, cat) => sum + cat.amount, 0)}
              decimalScale={2}
            />
          </p>
