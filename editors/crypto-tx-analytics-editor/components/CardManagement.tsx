...
export function CardManagement({ 
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "frozen": return "bg-amber-500";
        ...
        {cards.map((card) => (
          <div
            ...
            className={`relative overflow-hidden rounded-2xl p-5 text-white transition-all ${
              card.status === "frozen" ? "bg-gray-600" : "bg-gradient-to-br from-violet-600 to-purple-700"
            }`}
          >
              ...
              <div className="flex items-start justify-between mb-8">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`w-2 h-2 rounded-full ${getStatusColor(card.status)}`} />
                    <span className="text-white/80 text-sm font-medium capitalize">{card.status}</span>
                  </div>
                  <p className="text-white/60 text-xs">{card.type === "virtual" ? "Virtual Card" : "Physical Card"}</p>
                </div>
              ...
              <div className="flex items-end justify-between">
                ...
                {card.spendingLimit && (
                  <div className="text-right">
                    <p className="text-white/60 text-xs uppercase tracking-wider mb-1">Limit</p>
                    <p className="font-mono text-sm">{card.currency} {card.spendingLimit.toLocaleString()}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-700 mt-1">
              Your card is linked to your Gnosis Safe. Spend directly from your crypto wallet anywhere Visa is accepted.
            </p>
          </div>
        </div>
      </div>
