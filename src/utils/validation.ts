...
export function validateInputOrThrow<T>(
  ...
  if (!result.success) {
    const message = context
      ? `Validation error in ${context}: ${result.error.issues.map((e: { message: string }) => e.message).join(", ")}`
      : `Validation error: ${result.error.issues.map((e: { message: string }) => e.message).join(", ")}`;
